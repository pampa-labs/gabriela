...
from app.modules.openai_menu_inference import infer_menu_from_image, MenuItem
...
async def validate_image_url(image: ImageURL):
    ...
    if not any(image.url.path.lower().endswith(ext) for ext in allowed_extensions):
        raise HTTPException(status_code=400, detail="Invalid image file format. Allowed formats: jpg, jpeg, png, webp")
    return image
...
async def infer_menu(request: Request, image: ImageURL = Depends(validate_image_url), background_tasks: BackgroundTasks):
    try:
        ...
        raise HTTPException(status_code=500, detail="An unexpected error occurred during menu inference")
